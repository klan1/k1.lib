...
class k1_HtmlTableV3 {
    ...
    function __construct($boarID = null) {
        ...
        $this->class = "k1-table ui-responsive table-stroke";
        $this->other_attribs = "data-role='table' data-mode='reflow' ";
...
HTML;
    ...
    function doCode(net_klan1_dev_controllerClass $controllerObject = null) {
...
//        }
        $this->html_code.= "<form name = 'serach' action = '' method = 'post'>\n";
        $this->html_code.= "<TABLE data-role = 'table' data-mode = 'reflow' ";
        if ($this->id != '') {
        ...
        if ($this->htmlTableMode == "SQL") {
            ...
            if ($this->doSearchSystem) {
                ...
                foreach ($this->sqlResult[0] as $rowFieldName) {
                    ...
                    $this->html_code.= "\t\t<th><input type='text' name='{$rowFieldName}' id='{$rowFieldName}' value=\"{$formValue}\" data-mini='true' /></th>\n";
                }
                //unset the fields titles
                // </tr>
                $this->html_code.= "\t\t</tr>\n";
...
//                        $rowValue = number_format($rowValue);
                    ...
                    if (array_key_exists($rowFieldName, $this->tableFieldLinksArray)) {
                        ...
                        if ($controllerObject !== null) {
                            $sprintf_url = $controllerObject->parseUrlTag($sprintf_url);
                        }
        ...
        if ($this->htmlTableMode == "SQL") {
            $this->html_code.= "\t\t<button type='submit'  class='hiddeme' style='display:none'>Buscar</button></form>\n";
        }
            ...
            foreach ($tableFieldLinksArray as $key => $value) {
                if ($value === null) {
                    ...
                    if (is_object($controllerObject)) {
                        $value = $controllerObject->parseUrlTag($value);
                    }
