...
/*
     ...
     * @return array
     */
    public function get_db_table_config($table, $recursion = TRUE, $use_cache = TRUE) {

        // SQL to get info about a table
        $columns_info_query = "SHOW FULL COLUMNS FROM `{$table}`";
        $error = '';
        $columns_info_result = $this->sql_query($columns_info_query, TRUE, FALSE, $use_cache, $error);
...
AND table_name = '{$table}'";
        $INFORMATION_SCHEMA_result = $this->sql_query($INFORMATION_SCHEMA_query, TRUE, FALSE, $use_cache);
        ...
        foreach ($columns_info_result as $field_row) {
            ...
            if (!isset($field_config['label'])) {
                $field_config['label'] = $this->get_field_label_default($table, $field_name);
            }
            // ENUM FIX
            ...
            if (!empty($INFORMATION_SCHEMA_result)) {
                foreach ($INFORMATION_SCHEMA_result as $info_row) {
                    if (!empty($info_row['POSITION_IN_UNIQUE_CONSTRAINT']) && ($info_row['COLUMN_NAME'] == $field_name)) {
                        ...
                        if ($recursion) {
                            // RECURSION FIX 
                            $referenced_table_config = $this->get_db_table_config($info_row['REFERENCED_TABLE_NAME'], ($info_row['REFERENCED_TABLE_NAME'] != $table ? $recursion : FALSE));
     ...
     * @return boolean
     */
    public function sql_update($table, $data, $table_keys = array(), $db_table_config = array(), &$error_data = null, &$sql_query = null) {
        ...
        if ($this->is_enabled()) {
            if (is_array($data)) {
                ...
                if ($insert) {
                    ...
                    $last_insert_result = $this->sql_query($last_insert_sql, FALSE);
     ...
     * @return type
     */
    public function array_to_sql_set(array $array, $use_nulls = true, $for_where_stament = FALSE, $precise = TRUE) {
        if (is_array($array) && (count($array) >= 1)) {
     ...
     * @return type
     */
    function array_to_sql_set_exclude(array $array, $use_nulls = true, $for_where_stament = FALSE, $precise = TRUE) {
        if (is_array($array) && (count($array) >= 1)) {
    ...
    public function get_db_table_label_fields_from_row($row_data, $db_table_config) {
        if (!is_array($db_table_config)) {
            die(__FUNCTION__ . ": need an array to work on \$db_table_config");
        }
        $p = 0;
    ...
    function get_fk_field_label($fk_table_name, array $url_key_array = [], $source_table_config = []) {
    ...
    function sql_value_increment($table, $key_name, $key_value, $field_name, $step = 1) {
    ...
    function sql_table_count($table, $condition = "", $use_memcache = FALSE, $expire_time = 60) {
    ...
    function get_table_definition_as_array($table_name) {
        $definition = $this->sql_query("SHOW CREATE TABLE `{$table_name}`", FALSE);
