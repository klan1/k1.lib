...
/**
 * From V0.8 depreated no you have to use k1_get_field_label() instead
 * @param type $table_config_array
 * @return null
 */
function k1_get_table_label(&$table_config_array) {
    d("From V0.8 depreated no you have to use k1_get_field_label() instead");
    if (!is_array($table_config_array)) {
        die(__FUNCTION__ . ": need an array to work on \$table_config_array");
    }
    foreach ($table_config_array as $field => $config) {
...
function k1_get_fk_field_label($fkFieldName, $table_name, $url_key_array = Array()) {
    ...
    if (!empty($fkTableConfig[$fkFieldName]['label-field'])) {
        $fkWhereCondition = k1_table_keys_to_where_condition($url_key_array, $fkTableConfig);
        $fkSqlQuery = "SELECT {$fkTableConfig[$fkFieldName]['label-field']} FROM $table_name WHERE $fkWhereCondition";
        $sql_result = k1_sql_query($db, $fkSqlQuery, false);
        return $sql_result[$fkTableConfig[$fkFieldName]['label-field']];
    } else {
        return null;
    }
}
...
function k1_table_url_text_to_keys($url_text, $table_config_array) {
    ...
    if (!empty($errors)) {
        d($key_data);
        d($errors);
        die(__FUNCTION__ . ": Value types on the received \$url_text do not match with \$table_config_array");
    }
    return $key_data;
}
 ...
 */
function k1_sql_query(&$db, $sql, $return_all = true, $do_fields = false) {
    k1_db_check_object_type($db, __FUNCTION__);
...
function k1_sql_insert(&$db, $table, $data) {
    /*
...
function k1_array_to_sql_values($array) {
    if (is_array($array) && (count($array) > 1)) {
        ...
        k1_show_error("Bad formated array in " . __FUNCTION__);
        exit();
    }
}
